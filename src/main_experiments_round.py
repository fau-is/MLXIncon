    ...
    for train, test in sk.split(X=X, y=y):
        ...
        if "mlp" in mode:
            ...
            with torch.no_grad():
                ...
                if "mlp_flags_model" in mode:
                    y_train_pred = [np.round(p.item(), 0) for p in model(X_train, X_train[:, num_feat:])]
                    y_val_pred = [np.round(p.item(), 0) for p in model(X_val, X_val[:, num_feat:])]
                    y_test_pred = [np.round(p.item(), 0) for p in model(X_test, X_test[:, num_feat:])]
                else:
                    y_train_pred = [np.round(p.item(), 0) for p in model(X_train)]
                    y_val_pred = [np.round(p.item(), 0) for p in model(X_val)]
                    y_test_pred = [np.round(p.item(), 0) for p in model(X_test)]
        else:
            y_train_pred = [np.round(p, 0) for p in model.predict(X_train)]
            y_val_pred = [np.round(p, 0) for p in model.predict(X_val)]
            y_test_pred = [np.round(p, 0) for p in model.predict(X_test)]
        idx = idx + 1
...
if __name__ == "__main__":
    ...
    # "lr_flags_data", "lr_no_flags", "lasso_flags_data", "lasso_no_flags", "ridge_flags_data", "ridge_no_flags"
    # "lr_flags_data", "lr_no_flags", "lasso_flags_data", "lasso_no_flags", "ridge_flags_data", "ridge_no_flags", "mlp_no_flags", "mlp_flags_data", "mlp_flags_model"
    modes = ["lr_flags_data", "lr_no_flags", "lasso_flags_data", "lasso_no_flags", "ridge_flags_data", "ridge_no_flags",
             "mlp_no_flags", "mlp_flags_data", "mlp_flags_model"]
