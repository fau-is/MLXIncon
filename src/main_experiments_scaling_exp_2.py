...
def evaluate(X, y, seed, hpos, hpo, num_feat, data_set_name, mode, measure):
    ...
    X_train_val, X_test, y_train_val, y_test = train_test_split(X, y, test_size=0.5, shuffle=False, random_state=seed)

    X_train, X_val, y_train, y_val = train_test_split(X_train_val, y_train_val, test_size=0.1111, shuffle=True, random_state=seed)  # 0.1111
...
if __name__ == "__main__":
    ...
    data_set_names = [
        "../datasets/1T-kbs__max-6-atoms__max-5-elements__max-6-atoms__max-15-elements__with-flags.txt",
    ]
