...
def evaluate(X, y, seed, hpos, hpo, num_feat, data_set_name, mode, measure):
    ...
    for step in range(0, steps):
        ...
        for train, test in sk.split(X=X, y=y):
            ...
            num_rows = int(len(X_train) / steps)

            X_train = X_train[0:(step + 1) * num_rows, :]
            y_train = y_train[0:(step + 1) * num_rows]
        ...
        if not os.path.exists(f'../res/results_{data_set_name}_{step}.csv'):
            with open(f'../res/results_{data_set_name}_{step}.csv', 'w') as fd:
        ...
        with open(f'../res/results_{data_set_name}_{step}.csv', 'a') as fd:
            fd.write(
                f'{mode};{seed};{hpo};'
...
if __name__ == "__main__":
    ...
    hpo = True
    seed = 42
    modes = ["lr_flags_data", "ridge_flags_data", "lasso_flags_data", "mlp_flags_data", "mlp_flags_model"]
    torch.manual_seed(seed)
    np.random.seed(seed=seed)

    data_set_names = [
        "../datasets/datasets_2025_2/10Tkb__max-6-atoms__max-10-elements.txt",
    ]

    hpos = {
        # "mlp": {"learning_rate": [0.001], "weight_decay": [0.004], "hidden_size": [32]},
        "mlp": {"learning_rate": [0.001, 0.002, 0.003],
