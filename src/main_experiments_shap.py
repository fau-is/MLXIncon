...
def preprocess_data(dataset_name):
    ...
    feat_names = list(dict(sorted(vectorizer.vocabulary_.items())))
    feat_names = [feat.replace("sconsistent", "consistent") for feat in feat_names]

    return X, y, feat_names


def evaluate(X, y, seed, hpos, hpo, num_feat, data_set_name, mode, feat_names):
    """
    ...
    for train, test in sk.split(X=X, y=y):
        ...
        def f(X):
            if "mlp_flags_model" in mode:
                return model(torch.tensor(X), torch.tensor(X[:, num_feat:])).detach().numpy().flatten()
            else:
                return model(torch.tensor(X)).detach().numpy().flatten()

        # SHAP plot
        explainer = shap.KernelExplainer(f, X_train.detach().numpy())
        shap_values = explainer(X_test[1].detach().numpy())
        shap_values.feature_names = feat_names

        shap.plots.bar(shap_values, show=False)
        plt.savefig("shap.pdf", dpi=300, bbox_inches='tight')
        print(0)
...
if __name__ == "__main__":
    ...
    hpo = True
    seed = 42
    modes = ["mlp_flags_model"]  #["mlp_no_flags", "mlp_flags_data", "mlp_flags_model"]
    ...
    for data_set_name in data_set_names:
        for mode in modes:
            X, y, feat_names = preprocess_data(data_set_name)
            ...
            evaluate(X, y, seed, hpos, hpo, num_feat, data_set_name, mode, feat_names)